...
Toolbar 中的“手机打开”按钮会生成当前页面地址的二维码，手机扫码即可打开同一个应用页面。同源部署环境下，打开后可继续使用浏览器的 PWA 安装能力添加到主屏幕。
...
如果二维码地址是 `localhost` 或 `127.0.0.1`，手机通常无法直接访问，请改用电脑的局域网 IP 或部署后的 HTTPS 地址。扫码只会打开应用页面，笔记数据存储在各自浏览器的 IndexedDB 中，不会自动同步，也不会分享当前笔记内容。
